...
async def assign_task(agent_id: str, req: models.TaskRequest, current_user: str = Depends(auth.get_current_user)):
    ...
    system_prompt = (
        ...
        f"ESCALATION RULES — evaluate the task first, then output exactly ONE action token enclosed in square brackets on line 1:\n"
        ...
        f"IMPORTANT formatting rule: You MUST enclose the action token in square brackets on line 1 (e.g. [CONTINUE]). If you choose [CONTINUE] or [RISKY:reason], you MUST proceed to write your complete task response starting on a new line (after the token). Do not output ONLY the token.\n\n"
