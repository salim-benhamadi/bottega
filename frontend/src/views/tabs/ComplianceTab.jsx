...
const RISK_CONFIG = {
  'High Risk':    { bg: 'bg-rose-50',    border: 'border-rose-200',   text: 'text-rose-700',   badge: 'bg-rose-50 text-rose-700 border-rose-200',   dot: 'bg-rose-500' },
  'Limited Risk': { bg: 'bg-amber-50',   border: 'border-amber-200',  text: 'text-amber-700',  badge: 'bg-amber-50 text-amber-700 border-amber-200',  dot: 'bg-amber-500' },
  'Minimal Risk': { bg: 'bg-emerald-50', border: 'border-emerald-200',text: 'text-emerald-700',badge: 'bg-emerald-50 text-emerald-700 border-emerald-200',dot: 'bg-emerald-500' },
};
...
function SectionHeader({ number, title, count }) {
  return (
    <div className="flex items-center gap-3 mb-1">
      <div className="w-7 h-7 bg-blue-900 text-white flex items-center justify-center text-xs font-bold shrink-0">
        {number}
      </div>
      <h2 className="text-base font-semibold text-slate-900 leading-tight flex-1 min-w-0">{title}</h2>
      {count != null && (
        <span className="bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-medium px-2 py-0.5">{count}</span>
      )}
    </div>
  );
}
...
function RiskBadge({ level }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1 text-[10px] font-medium px-2 py-0.5 border uppercase tracking-widest ${cfg.badge}`}>
...
function StatCard({ value, label, sub, color = 'slate' }) {
  const colors = {
    ...
    blue:    'bg-white border-blue-200 text-blue-800',
  ...
  return (
    <div className={`border p-4 text-center ${colors[color]}`}>
      <p className="text-3xl font-bold leading-tight tabular-nums">{value}</p>
      <p className="text-[10px] font-medium uppercase tracking-widest mt-1 text-slate-500">{label}</p>
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-5">
        <div className="w-10 h-10 border-2 border-blue-200 border-t-blue-700 rounded-full animate-spin" />
        <p className="text-slate-500 text-sm font-medium">Generating compliance report…</p>
      </div>
    );
  }
      ...
      <div>
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-3">
            <div className="w-7 h-5 bg-blue-800 flex items-center justify-center shrink-0">
              <span className="text-[7px] text-yellow-300 font-bold tracking-tighter">★★★</span>
            </div>
            <span className="text-[10px] font-medium text-blue-700 uppercase tracking-widest">EU AI Act 2024 — Regulation (EU) 2024/1689</span>
          </div>
          <p className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mb-2">Compliance</p>
          <h1 className="text-3xl font-bold text-slate-950 mb-2 tracking-tight">Compliance Report</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={s.n} className="bg-white border border-slate-200 p-5">
              <p className="text-2xl font-bold text-slate-200 mb-2 tabular-nums">{s.n}</p>
        ...
        <button
          onClick={onGenerate}
          className="bg-blue-800 text-white px-8 py-3 text-sm font-medium hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-2.5">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-start justify-between gap-6 print:mb-6">
        <div>
          <div className="flex items-center gap-2 mb-3">
            ...
            <div>
              <p className="text-[10px] font-medium text-blue-700 uppercase tracking-widest">EU Artificial Intelligence Act</p>
            </div>
          </div>
          <p className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mb-2">Compliance</p>
          <h1 className="text-3xl font-bold text-slate-950 mb-2 tracking-tight">AI Compliance Report</h1>
          <div className="flex items-center flex-wrap gap-3 mt-1">
            <span className="text-xs text-slate-500">Organisation: <strong className="text-slate-800">{report.deployer_org}</strong></span>
            <span className="w-1 h-1 rounded-full bg-slate-300" />
            <span className="text-xs text-slate-500">Generated: <strong className="text-slate-800">{new Date(report.generated_at).toLocaleString()}</strong></span>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0 print:hidden">
          <button onClick={onGenerate} className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-2.5 text-sm font-medium hover:border-slate-300 transition-all">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            Refresh
          </button>
          <button onClick={handleExportCSV} className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-2.5 text-sm font-medium hover:border-slate-300 transition-all">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
            CSV
          </button>
          <button onClick={handleExportJSON} className="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-2.5 text-sm font-medium hover:border-slate-300 transition-all">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
            JSON
          </button>
          <button onClick={() => window.print()} className="flex items-center gap-2 bg-blue-800 text-white px-4 py-2.5 text-sm font-medium hover:bg-blue-700 transition-all">
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">All AI agents deployed on this account and their technical metadata.</p>
          </div>
              ...
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50">
                  {['Agent ID', 'Name', 'Role', 'Underlying Model', 'Autonomy', 'Risk Level', 'Tasks', 'Deployed'].map(h => (
                    <th key={h} className="text-left px-4 py-3 text-[10px] font-medium text-slate-400 uppercase tracking-widest whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(inventory || []).map((a, i) => (
                  <tr key={a.id} className={`border-b border-slate-50 hover:bg-slate-50/60 transition-colors ${i % 2 !== 0 ? 'bg-slate-50/30' : ''}`}>
                    ...
                    <td className="px-4 py-3 font-semibold text-slate-900 whitespace-nowrap">{a.name}</td>
                    <td className="px-4 py-3 text-slate-600">{a.role}</td>
                    <td className="px-4 py-3 text-slate-500">{a.underlying_model}</td>
                    <td className="px-4 py-3">
                      <span className={`text-[9px] font-medium px-1.5 py-0.5 border uppercase tracking-widest ${a.autonomy_mode.includes('Probation') ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
                ...
                {(!inventory || inventory.length === 0) && (
                  <tr><td colSpan={8} className="px-4 py-8 text-center text-slate-400">No agents deployed yet.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">Purpose, capabilities, and data access scope for each deployed AI agent.</p>
          </div>
          <div className="p-6 space-y-2">
            {(inventory || []).map(a => (
              <div key={a.id} className="border border-slate-200 overflow-hidden">
                <button
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 bg-blue-50 border border-blue-100 flex items-center justify-center text-blue-700 font-bold text-xs shrink-0">
                      {a.name.charAt(0)}
                    </div>
                    <div>
                ...
                {expandedAgent === a.id && (
                  <div className="px-4 pb-4 bg-slate-50 border-t border-slate-100">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4">
                      <div>
                        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1.5">Skills & Capabilities</p>
                        <div className="flex flex-wrap gap-1">
                          {a.skills.map(s => <span key={s} className="bg-white border border-slate-200 text-slate-600 text-[10px] font-medium px-2 py-0.5">{s}</span>)}
                          {a.skills.length === 0 && <span className="text-slate-400 text-xs">—</span>}
                        </div>
                      </div>
                      <div>
                        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1.5">Use Cases</p>
                        <div className="flex flex-wrap gap-1">
                          {a.use_cases.map(u => <span key={u} className="bg-white border border-slate-200 text-slate-600 text-[10px] font-medium px-2 py-0.5">{u}</span>)}
                          {a.use_cases.length === 0 && <span className="text-slate-400 text-xs">—</span>}
                        </div>
                      </div>
                      <div>
                        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1.5">Connected Tools</p>
                        <p className="text-xs text-slate-600">{a.allowed_tools.length > 0 ? a.allowed_tools.join(', ') : 'No tools assigned'}</p>
                      </div>
                      <div>
                        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1.5">Data Processed</p>
                        <p className="text-xs text-slate-600">{a.data_processed}</p>
                      </div>
                      <div>
                        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1.5">EU Data Residency</p>
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 border ${a.eu_data_residency ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
                          {a.eu_data_residency ? 'Enabled' : 'Not guaranteed'}
                        </span>
                      </div>
                      <div>
                        <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1.5">Audit Logging</p>
                        <span className={`text-[10px] font-medium px-1.5 py-0.5 border ${a.audit_log_enabled ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-rose-50 text-rose-700 border-rose-200'}`}>
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">Aggregate statistics on AI-processed tasks and human oversight engagement.</p>
          </div>
          <div className="p-6">
            ...
            <div className="bg-blue-50 border border-blue-100 p-4">
              <p className="text-xs font-medium text-blue-700 mb-2">Human Oversight Rate</p>
              {(() => {
                ...
                return (
                  <>
                    <div className="h-1.5 bg-blue-100 overflow-hidden mb-1.5">
                      <div className="h-full bg-blue-600 transition-all" style={{ width: `${Math.min(pct, 100)}%` }} />
                    </div>
                    <p className="text-xs text-blue-600">{pct}% of tasks involved human oversight ({overseen} / {total})</p>
                  </>
                );
              })()}
            </div>
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">Tasks that required or received explicit human review — evidence of oversight compliance.</p>
          </div>
          <div className="p-6">
            {(!approvalsShown || approvalsShown.length === 0) ? (
              <div className="py-8 text-center text-slate-400 border border-dashed border-slate-200">
                <p className="text-sm">No approval events recorded yet.</p>
              ...
              <>
                <div className="space-y-2">
                  {approvalsShown.map((t, i) => (
                    <div key={t.task_id || i} className={`border p-4 ${t.status === 'approved' ? 'bg-emerald-50 border-emerald-100' : 'bg-amber-50 border-amber-100'}`}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-semibold text-slate-800 mb-1">{t.task_description || '(no description)'}</p>
                          <p className="text-[10px] text-slate-500">Agent: {t.agent_name} · {t.timestamp ? new Date(t.timestamp).toLocaleString() : '—'}</p>
                        </div>
                        <span className={`shrink-0 text-[9px] font-medium px-2 py-0.5 border uppercase tracking-widest ${t.status === 'approved' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-amber-50 text-amber-700 border-amber-200'}`}>
                          {t.status}
                        </span>
                      </div>
                    </div>
                ...
                {(approval_records?.length || 0) > 6 && (
                  <button onClick={() => setShowAllApprovals(v => !v)} className="mt-4 w-full text-xs text-blue-700 font-medium py-2 hover:bg-blue-50 transition-colors border border-blue-100">
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">Chronological record of significant AI system events for traceability and accountability.</p>
          </div>
          <div className="p-6">
            {(!audit_events || audit_events.length === 0) ? (
              ...
              <>
                          ...
                          <div className="flex items-start gap-3">
                            <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 flex-wrap mb-0.5">
                                <span className={`text-[9px] font-medium px-1.5 py-0.5 border uppercase tracking-widest ${cfg.label}`}>
                                  {e.event.replace(/_/g, ' ')}
                                </span>
                ...
                {(audit_events?.length || 0) > 8 && (
                  <button onClick={() => setShowAllAudit(v => !v)} className="mt-5 w-full text-xs text-blue-700 font-medium py-2 hover:bg-blue-50 transition-colors border border-blue-100">
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">Each agent classified according to EU AI Act risk tiers based on role and use cases.</p>
          </div>
          <div className="p-6 space-y-3">
            {['High Risk', 'Limited Risk', 'Minimal Risk'].map(level => {
              ...
              return (
                <div key={level} className={`border ${cfg.border} ${cfg.bg} p-4`}>
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`w-2 h-2 rounded-full ${cfg.dot}`} />
                    <h3 className={`text-sm font-semibold ${cfg.text}`}>{level} <span className="font-normal opacity-70">({agents.length} agent{agents.length !== 1 ? 's' : ''})</span></h3>
                  </div>
                  <p className={`text-[10px] ${cfg.text} opacity-80 leading-relaxed mb-3`}>{agents[0]?.risk_rationale}</p>
                  <div className="space-y-2">
                    {agents.map(a => (
                      <div key={a.id} className="bg-white/70 border border-white px-3 py-2 flex items-center justify-between gap-2">
                        <div>
                        ...
                        <span className={`text-[9px] font-medium px-1.5 py-0.5 border uppercase tracking-widest ${a.eu_data_residency ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-slate-50 text-slate-500 border-slate-200'}`}>
        ...
        <div className="bg-white border border-slate-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100">
            ...
            <p className="text-xs text-slate-400 mt-2">Obligations under EU AI Act 2024 allocated between Bottega (provider) and {report.deployer_org} (deployer).</p>
          </div>
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
              <div className="border border-blue-100 bg-blue-50/40 p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <p className="text-sm font-semibold text-blue-900">Provider — Bottega</p>
                </div>
              ...
              <div className="border border-slate-200 bg-slate-50/40 p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <p className="text-sm font-semibold text-slate-800">Deployer — {report.deployer_org}</p>
                </div>
                <ul className="space-y-2.5">
            ...
            <div className="border border-emerald-100 bg-emerald-50/40 p-5">
              <div className="flex items-center gap-2 mb-4">
                <div className="w-7 h-7 bg-emerald-600 flex items-center justify-center shrink-0">
                ...
                <p className="text-sm font-semibold text-emerald-900">Human Oversight Measures</p>
              </div>
              <ul className="space-y-2.5">
